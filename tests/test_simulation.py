...
def test_simulation_cost_applied_for_zero_gross_return_trade():
    df = pd.DataFrame([
        {'sample_id':'a','stock_code':'x','target_date':'2024-01-01','target_open':100,'target_close':100,'y_true':1,'y_proba':0.6,'signal':1},
    ])
    t = Backtester().run(df, transaction_cost_bps=10)
    assert round(t.iloc[0]['gross_return'], 6) == 0.0
    assert round(t.iloc[0]['net_return'], 6) == -0.001
